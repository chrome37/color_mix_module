/*L6470 2台目コントロール　コマンド
 ...
 L6470_setparam_fnslpacc3(val); //[R, WH]逆起電力補償高速時加速勾配default 0x29 (8bit) (0.000015*val[% s/step])
...
void L6470_transfer3(int add,int bytes,long val){//2台目に送信
  int data[3];
  //while(!busy_flag3()){} //BESYが解除されるまで待機
...
void L6470_send3(unsigned char add_or_val){//2台目に送信
  digitalWrite(PIN_SPI_SS, LOW); // ~SSイネーブル。
  SPI.transfer(0); // 0送信。
...
long L6470_getparam3(int add,int bytes){//2台目に送信
  long val=0;
